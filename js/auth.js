/* auth.js — Login, signup, logout, privacy, banned screen, session */
    ...
    async function syncUserSession(user) {
      ...
      if (user) {
        ...
        // Same await pattern as withdrawals — load lifetime earnings before any Profile render
        if (typeof fetchUserEarningsHistory === 'function') {
          await fetchUserEarningsHistory();
        } else if (typeof fetchUserPostbacksHistory === 'function') {
          await fetchUserPostbacksHistory();
        }
        if (typeof fetchUserNotificationsFromServer === 'function') {
          await fetchUserNotificationsFromServer();
        }
        if (typeof subscribePostbacksRealtime === 'function') {
          subscribePostbacksRealtime();
        }
        if (typeof subscribeUserNotificationsRealtime === 'function') {
        ...
        if (typeof unsubscribeUserNotificationsRealtime === 'function') {
          unsubscribeUserNotificationsRealtime();
        }
        userCoins = 0;
        userNotifications = [];
        userWithdrawals = [];
